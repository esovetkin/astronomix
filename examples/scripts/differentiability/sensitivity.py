...
def run_adjoint_test(dim, ic_type, compare_backends=False):
    ...
    fig.tight_layout()
    grad_plot_path = str(figures_dir / f"gradient_{dim}d_{ic_type}.svg")
...
def run_gradient_convergence_test():
    ...
    plot_path = str(figures_dir / "gradient_convergence_test.svg")
...
if __name__ == "__main__":
    # Paper figure gradient_3d_gaussian.svg: the 3D Gaussian sensitivity with the
    # multi-backend (FD JAX / FD Pallas / FV JAX) AD-vs-exact-Fourier comparison.
    run_adjoint_test(3, 'gaussian', compare_backends=True)

    # Paper figure gradient_convergence_test.svg: AD-gradient convergence to the
    # exact analytical Fourier operator across resolutions.
